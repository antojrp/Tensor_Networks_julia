...
let
    io = open("resultados/Overlap_t$(BLAS.get_num_threads()).txt","w")
    print("Qubit    tiempo\n")
    close(io)
    nsim=5
    Num=2
    ...
    for p in 1:12
        max=2^p
        io = open("resultados/Overlap_t$(BLAS.get_num_threads()).txt","a")
        write(io,"D:$max   \n")
        close(io)
        for k in 1:m
            io = open("resultados/Overlap_t$(BLAS.get_num_threads()).txt","a")
            ...
            for j in 1:div(N-1,2)+(1-N%2)
                ...
                psi=random_mps(ComplexF64,i;linkdims=link)  
                inner(psi,phi)
            for j in 1:nsim
                phi=random_mps(ComplexF64,i;linkdims=link)  
                psi=random_mps(ComplexF64,i;linkdims=link)  
                t[j]=@elapsed inner(psi,phi)
            end
            media=0
            for k in 1:nsim
                media=media+t[k]
